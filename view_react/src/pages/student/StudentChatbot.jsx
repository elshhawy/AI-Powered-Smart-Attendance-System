// view-react/src/pages/student/StudentChatbot.jsx
import { useState, useRef, useEffect } from 'react'
import { motion, AnimatePresence } from 'framer-motion'
import { Send, Bot, User, Loader2, Trash2, Sparkles } from 'lucide-react'
import toast from 'react-hot-toast'
import { sendStudentChat } from '../../api'

const suggestions = [
  'What is my attendance rate?',
  'How many times have I been late?',
...
export default function StudentChatbot() {
  ...
  const send = async (text) => {
    ...
    try {
      ...
      const { data } = await sendStudentChat(msg, history)
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="page-subtitle mt-1">Ask anything about your own attendance</p>
        </div>
        {messages.length > 0 && (
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pr-2">
        {messages.length === 0 ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16">
            ...
            <p className="text-slate-500 text-sm mt-1">Ask about your own attendance, schedule, or stats</p>
            ...
            {messages.map((m, i) => (
              ...
              >
                <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center ${
                  m.role === 'user' ? 'bg-emerald-600' : 'bg-surface-800 border border-surface-700'
                }`}>
                  {m.role === 'user' ? <User size={14} className="text-white" /> : <Bot size={14} className="text-emerald-400" />}
                </div>
